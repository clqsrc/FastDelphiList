...
begin
  ...
  begin
      ...
      thread.OnRecv     := iocpClass.DoRecv;//OnRecv;
      thread.OnClose    := iocpClass.DoClose;//OnClose;
      thread.OnSend     := iocpClass.DoSend;//OnSend;
  ...
  threadFast.OnConnect := iocpClass.DoConnect;//OnConnect;
  threadFast.Resume;
    ...
    begin
      ...
      try
        ...
        begin
          ...
          if Assigned(OnClose) then OnClose(PerHandleData.Socket, PerHandleData.OuterFlag);//必须在 free 前调用
...
//     ll
           ...
           if Assigned(OnRecv) then OnRecv(PerHandleData.Socket, PerIoData.DataBuf.buf, BytesTransferred, PerHandleData.OuterFlag);
...
begin
  ...
  thread.OnConnect := Self.DoConnect;//OnConnect;
  thread.Resume;
